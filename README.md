...
- **权限管理**：权限覆盖放置、破坏、交互、功能方块、红石、爆炸、桶、传送、飞行、液体流入和领地时间；交互控制容器、门、活板门、栅栏门、床和普通右键，功能方块独立控制附魔台、切石机、砂轮、讲台等工作站。
- **成员与禁足**：支持 `/claim add`、`/claim unadd`、`/claim deny <玩家|*|全部>`、`/claim undeny <玩家|*|全部>`，被授权领地在列表中以只读/传送入口展示；旧版全局信任不再参与授权，移除成员时会同步清理旧全局残留。
...
| `/claim add <玩家>` | 添加成员 |
| `/claim unadd <玩家>` | 移除成员，并清理旧全局授权残留 |
...
| `/claim admin diagnose <领地名|#claimId> [--player 玩家]` | 查看跨服、传送和授权来源诊断 |
| `/claim admin add <玩家>` | 强制添加当前领地成员 |
| `/claim admin unadd <玩家>` | 强制移除当前领地成员，并清理旧全局授权残留 |
| `/claim admin deny <玩家|*|全部>` | 强制修改禁足 |
| `/claim admin undeny <玩家|*|全部>` | 强制取消禁足 |
| `/claim admin permission <permission> <allow|deny|unset>` | 修改权限，包含 `utility-interact`、`liquid-flow` 和 `time-cycle` |
...
| `coreclaim.admin.permission.manage` | `op` | 强制管理权限 |
...
| `rules.yml` | 新建普通领地和系统领地的默认权限 |
| `lang/zh_cn.yml` | 中文语言 |
| `lang/en_us.yml` | 英文语言 |
| `gui/*.yml` | 全部箱子菜单资源 |

旧版 `messages.yml` 会在启动时迁移到 `lang/zh_cn.yml`。旧版 `flags.yml` 与 `config.yml` 内的旧权限节点仍会兼容读取，但玩家侧统一通过 `/claim set` 的 GUI 管理权限。

## 默认规则摘要

- 普通领地默认拒绝放置、破坏、交互、功能方块、红石、爆炸、桶、传送、飞行。
- 系统领地默认允许传送，默认拒绝放置、破坏、交互、功能方块、红石、爆炸、桶、飞行。
- `interact` 控制容器、门、活板门、栅栏门、床和普通右键；`utility-interact` 控制附魔台、切石机、砂轮、讲台读书、制图台、锻造台、织布机、铁砧等功能方块；`redstone` 控制按钮、拉杆、压力板和红石类交互。
