...
public final class BlockProtectionListener implements Listener {
    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (event.isCancelled()) {
            sendPreCancelledCakeDenyIfNeeded(event, claim, clickedType);
            return;
        }
        ...
        if (claim.isPresent() && toolChangePermission != null) {
            ...
            if (support.isAxeStrippingWood(clickedType, event.getItem())) {
                if (!bypassing && !support.claimService().canAccess(claim.get(), event.getPlayer().getUniqueId())) {
                    event.setCancelled(true);
                    support.sendProtectionDeny(event.getPlayer(), claim.get());
                    return;
                }
    ...
    private void sendPreCancelledCakeDenyIfNeeded(PlayerInteractEvent event, Optional<Claim> claim, Material clickedType) {
        if (claim.isEmpty()
            || !support.isCakeConsumption(clickedType, event.getItem())
