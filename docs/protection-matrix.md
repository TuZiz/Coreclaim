...
| 原木去皮 | `PlayerInteractEvent` 右键方块 | `BREAK` | 无权限时取消事件 | 成功时记录建造活跃 |
...
| 盔甲架、命名牌、实体容器 | `PlayerInteractEntityEvent` / `PlayerArmorStandManipulateEvent` | 盔甲架为 `BREAK`，非生物实体容器为 `INTERACT`，其他非生物实体按手持命名牌推导 `BREAK` 或 `INTERACT` | `setCancelled(true)` | `BREAK` 记录建造活跃，其他记录交互活跃 |
| 栓绳、钓鱼竿实体交互 | `PlayerLeashEntityEvent` / `PlayerUnleashEntityEvent` / `HangingBreakByEntityEvent` / `PlayerFishEvent` | 栓绳/解栓领地内生物需要 `MOB_INTERACT`；钓鱼竿按目标实体推导权限；拴绳结破坏默认放行 | 无权限时取消；钓鱼可移除钩子 | 成功时记录交互活跃 |
| 液体跨界流入领地 | `BlockFromToEvent` | `LIQUID_FLOW`；不再跟随 `BUCKET` | 水、岩浆、气泡柱流入未允许的领地时 `setCancelled(true)` | 不记录活跃值 |
