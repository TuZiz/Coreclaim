...
当前玩家侧按“脚下当前领地优先”设计，公开入口如下：

| 命令 | 作用 | 权限 |
| --- | --- | --- |
| `/claim` | 打开领地核心菜单 | `coreclaim.use` |
| `/claim help` | 查看玩家帮助 | `coreclaim.use` |
| `/claim info` | 查看脚下当前领地详情 | `coreclaim.use` |
| `/claim list` | 查看自己的领地列表 | `coreclaim.use` |
| `/claim menu` | 打开当前领地菜单 | `coreclaim.use` |
| `/claim create <领地名>` | 用当前选区创建领地 | `coreclaim.use` |
| `/claim tp <领地名>` | 传送到可进入的领地 | `coreclaim.use` |
| `/claim tpset` | 将脚下位置设置为当前领地传送点 | `coreclaim.manage.tpset` |
| `/claim add <玩家>` | 在当前领地添加成员 | `coreclaim.use` |
| `/claim remove <玩家>` | 在当前领地移除成员 | `coreclaim.use` |
...
| `/claim flag` | 查看脚下当前领地交互细则 | `coreclaim.manage.flags` |
| `/claim flag <flag> <allow\|deny\|unset>` | 修改脚下当前领地单个交互细则 | `coreclaim.manage.flags` |
| `/claim delete <领地名>` | 删除自己的指定领地 | `coreclaim.use` |
| `/claim transfer ...` | 转让自己的领地 | `coreclaim.transfer` |

说明：

- `deny` 现在是纯命令管理项，不再提供 GUI 页面。
- 成员通过 `/claim add <玩家>` 添加后，默认拥有该领地全部权限。
- 权限页现在已经合并“基础权限 + 交互细则”，不再有单独的旗标页。
...
- 第一块普通领地创建成功后，会提示一次：以后第二块地开始，请直接用普通金锄头选区创建。
...
- 玩家背包里原本已有的普通金锄头，不会再被插件强制改名或改 lore。
...
- `claim-flags.yml` 已下线，功能已并入 `claim-permissions.yml`。
...
| `/claim admin diagnose <领地名\|#claimId>` | 查看跨服、TP 路由、规则来源、deny、交互细则摘要 | `coreclaim.admin.view` / `coreclaim.admin` |
...
| `/claim admin remove <玩家>` | 在当前领地强制移除成员 | `coreclaim.admin.member.manage` / `coreclaim.admin.force` / `coreclaim.admin` |
...
| `/claim givecore <玩家> [数量]` | 发放普通领地核心 | `coreclaim.admin.reward.givecore` / `coreclaim.admin.force` / `coreclaim.admin` |
...
- 活跃值系统仍保留展示、占位符和管理员调节能力，但不再作为圈地门槛。
